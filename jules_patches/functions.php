...
function get_notes_for_user($conn, $user_id, $filters = []) {
    ...
    $sql_get_goal = $conn->prepare("SELECT id FROM saving_goals WHERE name = ? AND user_id = ?");

    while ($row = $result->fetch_assoc()) {
        $row['linked_goal_id'] = null;
        if (!empty($row['transaction_description']) && strpos($row['transaction_description'], 'Contributo a: ') === 0) {
            $goal_name = str_replace('Contributo a: ', '', $row['transaction_description']);
            $sql_get_goal->bind_param("si", $goal_name, $user_id);
            $sql_get_goal->execute();
            $goal_result = $sql_get_goal->get_result();
            if ($goal = $goal_result->fetch_assoc()) {
                $row['linked_goal_id'] = $goal['id'];
            }
        }
        $notes[] = $row;
    }
...
function create_and_share_note_with_fund_members($conn, $note_content, $creator_id, $fund_id) {
    ...
    foreach ($members as $member) {
        if ($member['id'] != $creator_id) {
            $stmt_share->bind_param("ii", $member['id'], $member['id']);
            $stmt_share->execute();
        }
    }
    $stmt_share->close();
